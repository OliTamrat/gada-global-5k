...
export default function RegisterPage() {
  ...
  const inputClass =
    "w-full px-4 py-3.5 border-2 border-charcoal/8 rounded-xl bg-white focus:border-yellow focus:ring-2 focus:ring-yellow/10 outline-none transition-all text-[14px] placeholder:text-charcoal/25";
  const labelClass =
    "block text-[10px] font-bold tracking-[2px] uppercase text-charcoal/35 mb-2";
      ...
      <section className="relative pt-28 pb-14 px-6 md:px-16 lg:px-20 bg-charcoal overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto">
          <div className="flex items-center gap-3 mb-6">
            <Link href="/" className="text-white/30 text-[13px] hover:text-white/60 transition-colors no-underline">Home</Link>
            <span className="text-white/15">/</span>
            <span className="text-yellow text-[13px] font-semibold">Register</span>
          </div>

          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
            <div>
              <span className="text-[10px] font-bold tracking-[4px] uppercase text-yellow mb-4 block">Registration</span>
              ...
              <p className="text-[15px] leading-[1.8] text-white/40 max-w-[420px]">
            ...
            <div className="flex gap-8">
              {[
                ...
                <div key={stat.label} className="text-right">
                  ...
                  <div className="text-[10px] font-semibold tracking-[2px] uppercase text-yellow/60">{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
            ...
            <ScrollReveal>
              <div className="h-full flex flex-col">
                ...
                <div className="flex flex-col gap-3 mb-6">
                  {tiers.map((t) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${isActive ? "yellow-card" : "bg-charcoal/5 text-charcoal/30"}`}>
                          {t.id === "early" ? (
                ...
                <div className="bg-white rounded-2xl p-6 border border-charcoal/5 flex-1">
                  <h4 className="font-bold text-[13px] text-charcoal mb-5 tracking-tight">What&apos;s Included</h4>
                ...
                <div className="bg-charcoal rounded-2xl p-6 mt-5 text-white">
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      { icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>, label: "Start", value: "7:30 AM ET" },
                      { icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>, label: "Location", value: "Rock Creek, DC" },
                      ...
                      <div key={item.label} className="flex items-center gap-2.5">
                        ...
                        <div>
                          <div className="text-[9px] font-semibold text-white/25 uppercase tracking-wider">{item.label}</div>
            ...
            <ScrollReveal>
              <div className="bg-white rounded-3xl p-7 md:p-10 shadow-[0_8px_40px_rgba(0,0,0,0.06)] border border-charcoal/4 h-full flex flex-col">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="yellow-card px-4 py-2 rounded-xl font-bold text-[13px] tracking-tight shrink-0">
                    {tier?.name} &mdash; ${(tier?.price ?? 0) / 100}
                  </div>
                </div>

                {error && (
                  <div className="bg-red-50 text-red-700 px-4 py-3 rounded-xl text-[13px] mb-6 border border-red-200">
                    {error}
                  </div>
                )}
                ...
                <form onSubmit={handleSubmit} className="flex-1 flex flex-col">
                  ...
                  <div className="mt-auto pt-6">
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full py-4 yellow-card rounded-xl font-bold text-[14px] tracking-wider uppercase hover:-translate-y-0.5 hover:shadow-[0_8px_32px_rgba(245,200,66,0.25)] transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer border-none"
                    >
                      {loading ? "Processing..." : "Proceed to Payment"}
                    </button>

                    <p className="text-[11px] text-charcoal/25 text-center mt-3">
                ...
                <div className="mt-6 bg-charcoal/4 rounded-xl p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[13px] font-bold text-charcoal tracking-tight">Limited to 500 runners</div>
                      <div className="text-[11px] text-charcoal/35">Spots are filling up fast</div>
                    </div>
                  </div>
                  <Link href="/shop" className="text-[11px] font-bold text-gold-dim hover:text-charcoal transition-colors no-underline tracking-wider uppercase">
                    Shop Merch
                  </Link>
                </div>
              </div>
