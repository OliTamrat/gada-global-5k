    ...
    <main className="bg-charcoal min-h-screen pt-24 pb-20 px-6 md:px-16 lg:px-20">
      <div className="max-w-5xl mx-auto">
        <ScrollReveal className="text-center mb-10">
          ...
          <p className="text-[14px] text-white/50 max-w-[400px] mx-auto mb-2">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            ...
            <div key={s.label} className="dark-card rounded-2xl p-5 text-center">
              ...
              <div className="text-[11px] font-semibold text-white/40 uppercase tracking-wider mt-1">{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-[14px] placeholder:text-white/25 focus:outline-none focus:border-yellow/40 transition-colors"
            />
          </div>
          <div className="flex gap-2">
            {(["all", "male", "female"] as const).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-4 py-3 rounded-xl text-[12px] font-bold uppercase tracking-wider border-none cursor-pointer transition-all ${
                  filter === f
                    ? "yellow-card"
        ...
        {results.length === 0 && !loading && (
          <div className="text-center py-16">
            <p className="text-white/40 text-[14px] mb-4">No race data yet.</p>
            {!seeded && (
              <button
              ...
              {finished.map((r) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[14px] font-bold text-white truncate group-hover:text-yellow transition-colors">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <div className="text-[11px] text-white/35">{r.pace}/mi</div>
                  </div>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-white/20 group-hover:text-yellow transition-colors shrink-0">
        ...
        {running.length > 0 && (
          <div className="mb-8">
            <h3 className="text-[11px] font-bold tracking-[3px] uppercase text-green-light/60 mb-4">
            ...
            <div className="space-y-2">
              {running.map((r) => (
                <div key={r.bib} className="flex items-center gap-4 dark-card rounded-xl p-4">
                  ...
                  <div className="text-[13px] font-bold text-green-light tracking-tight">
                    Running...
                  </div>
                </div>
              ))}
        ...
        {waiting.length > 0 && (
          <div>
            <h3 className="text-[11px] font-bold tracking-[3px] uppercase text-white/20 mb-4">
            ...
            <div className="space-y-2">
              {waiting.map((r) => (
                <div key={r.bib} className="flex items-center gap-4 bg-white/2 rounded-xl p-4 border border-white/4">
                  <div className="w-10 h-10 rounded-xl bg-white/4 flex items-center justify-center text-white/20 font-bold text-[14px] shrink-0">
                    {r.bib}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[14px] font-medium text-white/30 truncate">
                      {r.firstName} {r.lastName}
                    </div>
                  </div>
                </div>
        ...
        <div className="flex gap-4 justify-center flex-wrap mt-12">
          ...
          <Link href="/race/disputes" className="bg-white/8 text-white px-6 py-3 rounded-xl font-bold text-[12px] tracking-wider uppercase border border-white/10 hover:border-yellow/40 hover:text-yellow transition-all no-underline">
            Dispute a Time
          </Link>
          <Link href="/" className="text-white/25 text-[13px] hover:text-yellow transition-colors no-underline flex items-center gap-2">
            &larr; Back to Home
          </Link>
        </div>
      </div>
